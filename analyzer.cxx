...
void analyzer::Loop() {
	...
	int thetavis_split = ThetaVisLabels.size();
	TH1D* TrueSplitThetaVisPlot[thetavis_split];
	...
	for (int inte = 0; inte < NInte; inte++) {
		...
		TrueThetaVisPlot[inte] = new TH1D(InteractionLabels[inte]+"TrueThetaVisPlot",";#theta_{vis} [deg]",36,0.,180.);
		TrueThetaNuPlot[inte] = new TH1D(InteractionLabels[inte]+"TrueThetaNuPlot",";#theta_{#nu_{#mu}} [deg]",36,0.,180.);
		TrueEnuPlot[inte] = new TH1D(InteractionLabels[inte]+"TrueEnuPlot",";E_{#nu} [GeV]",20,0.,1.5);
		TrueTheta_vsThetaVisPlot[inte] = new TH2D(InteractionLabels[inte]+"TrueTheta_vsThetaVisPlot",";#theta_{#nu_{#mu}} [deg];#theta_{vis} [deg]",36,0.,180.,36,0.,180.);

	} // End of the loop over the interaction processes					
	
	//----------------------------------------//

	// Loop over the theta_vis plit

	for (int is = 0; is < thetavis_split; is++) {

		TrueSplitThetaVisPlot[is] = new TH1D(ThetaVisLabels[is]+"TrueSplitThetaVisPlot",";#theta_{vis} [deg]",36,0.,180.);
	...
	for (Long64_t jentry=0; jentry<nentries;jentry++) {
		...
		for (int i = 0; i < ninitp; i++) {
			...
			if (pdg_init[i] == 14) {
					...
					TVector3 numu(px_init[i],py_init[i],pz_init[i]);
		...
		if ( ProtonTagging == 1 && ChargedPionTagging == 0 && NeutralPionTagging == 0 && MuonTagging == 1 && TrueHeavierMesonCounter == 0) { 
			...
			TrueSplitThetaVisPlot[0]->Fill(thetavis,weight);
			if ( TMath::Abs(numu_theta - thetavis) < theta_vis_open) { TrueSplitThetaVisPlot[1]->Fill(thetavis,weight); }
			else { TrueSplitThetaVisPlot[2]->Fill(thetavis,weight); }
